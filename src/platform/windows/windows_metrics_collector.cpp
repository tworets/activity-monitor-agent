...
namespace {
	...
	std::string getProcessName(HWND hWnd) {
		...
		// handle закрывается до проверки результата, чтобы освободить его в любом случае
		const BOOL nameRetrieved { QueryFullProcessImageNameW(handle, 0, buf.data(), &bufferSize) };
